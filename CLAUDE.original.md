...
- User-facing feature added/changed → update docs across ALL of `docs/` + README, not just `*.md`. Grep `docs/` for the feature area. Specifically: `README.md` Features list, `docs/roadmap.md` (move shipped items out of "Next"), and **`docs/index.html`** — the hand-maintained public landing page with a curated feature-card grid (append to an existing card, don't break the grid). `CHANGELOG.md` is release-please auto-generated — never hand-edit.
- Two settings modals edit the SAME `senda.meta.yaml`, don't mix them up: `CollectionSettings.tsx` = collection ROOT (opened from collection context menu → "Collection settings"; reads `collection()` from store), `FolderSettings.tsx` = sub-folders (opened from a folder's context menu; takes `props.path`). Collection-root-only config (proxy/TLS, default auth) lives in `CollectionSettings`. `FolderSettings` has an `isRoot`-style path check available but it's NEVER opened for the root — putting root-only UI behind a check there makes it unreachable. New bound `Git*`/meta App method touched by either modal → mirror it in `tests/visual/mock-backend.mjs` (see e2e note below).
- New field on `model.Collection` → add it to BOTH hand-copy sites in `store.go` (`ReadMeta` and `SaveCollection` copy fields explicitly, not via struct assignment) or it silently won't persist.
...
TWO mocks, don't confuse them: the **e2e** webServer runs `bun run dev:mock` (`vite --mode test` → `installDevMock`), so playwright specs hit **`frontend/src/lib/devMock.ts`**. `tests/visual/mock-backend.mjs` is the separate **visual screenshot** harness (`shoot.mjs`, `addInitScript`). A bound App method a spec calls (e.g. `PickFile`) must exist in **devMock.ts** or it returns undefined and the flow silently no-ops — a missing method does NOT error. Add it to mock-backend.mjs too only if the visual harness renders that path.
