...
- **Chaining & flows** — inline `{{res.<slug>...}}` response references, plus declarative `*.flow.yaml` graphs (request / branch / setvar / loop / parallel / delay) created/edited/deleted in-app (raw YAML, live validation) or by hand, runnable from the app or headless via `senda run -flow`; graphs are validated before they run.
